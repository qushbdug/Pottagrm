...
"""
            ...
            for network in networks:
                ...
                button_text = f"{status_icon} {name} - {provider} ({available_cards} كرت متاح)"
    ...
    try:
        ...
        cursor.execute('''
            SELECT n.id, n.name, n.provider, n.city, n.location, n.description, n.is_active, n.is_approved,
                   n.created_at, u.full_name as supplier_name, u.id as supplier_id,
                   COUNT(cc.id) as categories_count,
                   COUNT(c.id) as total_cards,
                   COUNT(CASE WHEN c.is_sold = 0 THEN 1 END) as available_cards,
                   COUNT(CASE WHEN c.is_sold = 1 THEN 1 END) as sold_cards
        ...
        network_id, name, provider, city, location, description, is_active, is_approved, created_at, supplier_name, supplier_id, categories_count, total_cards, available_cards, sold_cards = network
...
💳 **إحصائيات الكروت:**
• **فئات الكروت:** {categories_count}
• **إجمالي الكروت:** {total_cards}
• **متاحة:** {available_cards}
• **مباعة:** {sold_cards}
...
"""
        ...
        if categories:
            ...
            for i, category in enumerate(categories, 1):
                cat_name, cat_value, cat_price, cat_stock, cat_available = category
                status_cat = "✅" if cat_available else "❌"
                text += f"{i}. {status_cat} {cat_name} - {cat_value} - {cat_price} ريال ({cat_stock} كرت)\n"
        
        keyboard = [
            [InlineKeyboardButton('📝 تعديل الشبكة', callback_data=f'admin_edit_network_{network_id}'),
             InlineKeyboardButton('💳 إدارة الكروت', callback_data=f'admin_upload_to_network_{network_id}')],
            [InlineKeyboardButton('🔄 تغيير الحالة', callback_data=f'admin_toggle_network_{network_id}'),
             InlineKeyboardButton('📊 إحصائيات تفصيلية', callback_data=f'admin_network_stats_{network_id}')],
...
"""
        ...
        await query.edit_message_text(f"{EMOJIS['error']} حدث خطأ في تأكيد حذف الشبكة.")
...
async def admin_confirm_delete_network_handler(update: Update, context: CallbackContext, network_id: str):
    ...
    try:
        ...
        # حذف الشبكة وجميع البيانات المرتبطة (بترتيب المفاتيح الخارجية)
        try:
            # 1. حذف الكروت أولاً
            cursor.execute('''
                DELETE FROM cards 
            ...
            # 3. حذف العروض المرتبطة
            cursor.execute('DELETE FROM offers WHERE network_id = ?', (network_id,))
            ...
            # إنشاء قيد محاسبي لتسجيل الحذف
            try:
                from bot_modules.accounting_integration import create_accounting_entry
                create_accounting_entry('network_deletion', 0, user['id'], network_id, f"حذف شبكة: {network_name}")
...
"""
            ...
            await query.edit_message_text(
                f"❌ **فشل في حذف الشبكة**\n\n🔍 **السبب:** {str(db_error)}\n\n💡 **حاول مرة أخرى أو تواصل مع الدعم التقني**",
